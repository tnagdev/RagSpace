...
from src.graph.formatters import format_file_content_for_llm
...
async def image_content_node(state: AgentState, config: RunnableConfig) -> dict:
    """Sibling to video_content_node for IMAGE file types — calls the generic
    /embed/content/file endpoint (get_file_content) instead of the video-only
    /embed/content/video endpoint, since images have no scene timeline."""
    ...
    log = NodeLogger(logger, correlation_id, "image_content")
    log.info("start", file_count=len(file_ids))

    sse_events = [{"type": "step_start", "step": "image_content", "label": "Reading image content..."}]
    combined_parts: list[str] = []

    for fid in file_ids:
        log.info("fetching", file_id=fid)
        sse_events.append({"type": "tool_start", "tool": "get_file_content", "arguments": {"file_id": fid}})
        try:
            content_response = await asyncio.wait_for(
                file_embedder_service.get_file_content(file_id=fid, include_metadata=True),
                timeout=settings.file_content_timeout,
            )
            formatted = format_file_content_for_llm(content_response)
            chunk = json.dumps(formatted, indent=2)
            combined_parts.append(chunk)
            log.info("fetched", file_id=fid, context_chars=len(chunk))
            sse_events.append({"type": "tool_result", "tool": "get_file_content", "file_id": fid, "status": "ok"})
        except asyncio.TimeoutError:
            log.error("timeout", file_id=fid, timeout_s=settings.file_content_timeout)
            sse_events.append({
                "type": "step_error",
                "step": "image_content",
            ...
            sse_events.append({"type": "tool_result", "tool": "get_file_content", "file_id": fid, "status": "timeout"})
            ...
            sse_events.append({"type": "tool_result", "tool": "get_file_content", "file_id": fid, "status": "error"})

    combined_context = "\n\n".join(combined_parts)
    sse_events.append({
        "type": "step_done",
        "step": "image_content",
        "label": f"Retrieved content for {len(file_ids)} image(s)",
    })

    # Fetch the images themselves (not scene thumbnails — images have no scenes)
    # so the frontend can render a preview card, reusing the same scene_thumbnails
    # SSE event the frontend already generically handles.
    search_results: list[dict] = []
    if file_ids:
        sse_events.append({"type": "step_start", "step": "image_thumbnails", "label": "Loading image previews..."})
        try:
            search_response = await asyncio.wait_for(
                file_embedder_service.search(
                    query=user_message or "image",
                    user_id=user_id,
                    file_ids=file_ids,
                    max_results=len(file_ids) * 2,
        ...
        sse_events.append({
            "type": "step_done",
            "step": "image_thumbnails",
            "label": f"Loaded {len(search_results)} image(s)",
        })
